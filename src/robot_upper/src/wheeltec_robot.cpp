...
{
  ros::init(argc, argv, "robot_upper_node"); //ROS initializes and sets the node name //ROS初始化 并设置节点名称 
...
void turn_on_robot::Cmd_Vel_Callback(const geometry_msgs::Twist &twist_aux)
{
  ...
  Send_Data.tx[3] = transition;     //取数据的低8位
...
{
  tf::TransformBroadcaster odom_broadcaster;//发布里程计坐标变换
  while(ros::ok())
  {
